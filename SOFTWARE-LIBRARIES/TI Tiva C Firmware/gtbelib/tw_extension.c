 ...
 ******************/
/**
 * Initializes the Virtual communications port UART that runs through the ICDI and the USB to the PC.
 *
 *  \param SysClkFreq - clock frequency of the system.
 *
 *  \param baudRate - baud rate of the UART e.g. 115200 to connect to PC
 *	 The baudrate is limited by the PC Vcom driver
 *
 ...
 *   no parity
 **/
void twe_initVCOM(uint32_t SysClkFreq, uint32_t baudRate) {
	 MAP_SysCtlPeripheralEnable(TWE_VCOM_PERIPH);
	 MAP_SysCtlPeripheralEnable(TWE_VCOM_GPIO_PERIPH);

	 MAP_GPIOPinConfigure(TWE_VCOM_RX_PIN_CONFIG);
	 MAP_GPIOPinConfigure(TWE_VCOM_TX_PIN_CONFIG);
	 MAP_GPIOPinTypeUART(TWE_VCOM_GPIO_BASE, TWE_VCOM_RX_PIN | TWE_VCOM_TX_PIN);

	 MAP_UARTConfigSetExpClk(TWE_VCOM_BASE, SysClkFreq, baudRate,
