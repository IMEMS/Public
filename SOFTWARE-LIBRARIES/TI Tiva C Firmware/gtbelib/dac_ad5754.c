 ...
 **/
 ...
 /**
  * initializes SSI for use with the DAC
  **/
 void DAC_initSSI(uint32_t SysClkFreq) {
 	...
 	// Set the pin muxing for the DAC
 ...
 **/
 void DAC_updateDataVolt(uint32_t dacAddress, uint32_t rangeValue, _Bool bin,
	 ...
	 if(dacAddress == DAC_ADDR_NONE_AD) {
		 ...
		 switch(rangeValue) {
			 case DAC_RANGE_P5V     : data = (uint32_t) (131072 * voltage + 5)/10; break;		// NEED TO CHECK MATH
			 case DAC_RANGE_P10V    : data = (uint32_t) (65536 * voltage + 5)/10; break;		// NEED TO CHECK MATH
			 case DAC_RANGE_P10P8V  : data = (uint32_t) (327680 * voltage + 27)/54; break;		// NEED TO CHECK MATH
			 case DAC_RANGE_PM5V    : data = (uint32_t) (65536 * (voltage + 5))/10; break;		// NEED TO CHECK MATH
			 case DAC_RANGE_PM10V   : data = (uint32_t) (32768 * (voltage + 10) + 5)/10; break;	// NEED TO CHECK MATH
			 case DAC_RANGE_PM10P8V : data = (uint32_t) (81920 * voltage + 884763)/432; break;	// NEED TO CHECK MATH
			 default                : data = 0;
		 }
 ...
 *        This must be long enough to ensure that ~sync goes high between writes
 *
 *	\note dacAddress DAC_ADDR_NONE_AD sends a nop
 *
 *  \param dacAddress Selected DAC(s) from A through D
...
void DACd_initDAC(uint32_t rangeValue, uint32_t pwrSettingsValue, uint32_t SysClkFreq) {
	...
	DACd_setSettingsCtl((!DAC_CTL_SDO_DIS & DAC_CTL_TSD | DAC_CTL_CLAMP) |
	 			 	 	 	(DAC_CTL_TSD_EH | DAC_CTL_CLAMP_EH));
	MAP_SysCtlDelay(4000);
...
/**
 * initializes SSI for use with the daisy-chained DAC
 **/
  void DACd_initSSI(uint32_t SysClkFreq) {
  	uint32_t trashBin[1] = {0};
  	...
  	MAP_GPIOPinConfigure(DAC_SSI_RX_PIN_CONFIG);
  	MAP_GPIOPinConfigure(DAC_SSI_TX_PIN_CONFIG);
  	MAP_GPIOPinTypeSSI(DAC_SSI_GPIO_BASE,DAC_SSI_TX_PIN|DAC_SSI_RX_PIN|DAC_SSI_CLK_PIN);

  	// Setup the FSS pin for the framed writes required in daisychain mode
  	MAP_GPIOPinTypeGPIOOutput(DAC_SSI_GPIO_BASE, DAC_SSI_FSS_PIN);
  	MAP_GPIOPinWrite(DAC_SSI_GPIO_BASE, DAC_SSI_FSS_PIN, 2);
 ...
 **/
 void DACd_setSettingsCtl(uint32_t ctlSettingsValue) {
	 MAP_GPIOPinWrite(DAC_SSI_GPIO_BASE, DAC_SSI_FSS_PIN, 0);
	 ...
	 while(SSIBusy(DAC_SSI_BASE)){} // Wait until the end of the transmission
	 MAP_GPIOPinWrite(DAC_SSI_GPIO_BASE, DAC_SSI_FSS_PIN, 2);
 }
 ...
 **/
 void DACd_updateDataVolt(uint32_t dacAddress, uint32_t rangeValue,
	 ...
	 DAC_updateDataVolt(dacAddress & 0x000000FF, rangeValue & 0x000000FF, bin_AD, voltage_AD);
	 while(SSIBusy(DAC_SSI_BASE)){} // Wait until the end of the transmission
	 MAP_GPIOPinWrite(DAC_SSI_GPIO_BASE, DAC_SSI_FSS_PIN, 2);
 }
